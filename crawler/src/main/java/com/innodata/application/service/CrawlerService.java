...
public class CrawlerService {
    ...
    public ResponseEntity<HashMap<String, Object>> uploadFileToDropbox(MultipartFile multipartFile) throws IOException, DbxException {
        ...
        if (ACCESS_TOKEN == null || ACCESS_TOKEN.isEmpty()) {
        	return applicationUtils.generateErrorResponse("❌ Error: Dropbox Access Token is not set!", HttpStatus.BAD_REQUEST);
        }
        ...
        try {
            ...
            if (!e.getMessage().contains("conflict")) { // Avoid redundant folder creation errors
            	return applicationUtils.generateErrorResponse("❌ Error: Folder creation failed (may already exist)!", HttpStatus.CONFLICT);
            }
        }
